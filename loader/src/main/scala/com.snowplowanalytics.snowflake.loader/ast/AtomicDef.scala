...
object AtomicDef {
  ...
  /** Compare columns fetched from a real table with expected in atomic definition */
  def compare(columns: List[Either[String, Column]]): List[String] =
    columns
      .zipAll(AtomicDef.columns.map(_.asRight), MissingExisting.asLeft, MissingAtomic.asLeft)
      .zipWithIndex
      .flatMap { case ((existing, atomic), i) =>
        val position = i + 1   // indexes are 0-based
        (existing, atomic) match {
          case (Right(e), Right(a)) if e == a =>   // Match
            Nil
          case (Right(e), Right(a)) =>             // Different
            List(s"Existing column [${e.show}] doesn't match expected definition [${a.show}] at position $position")
