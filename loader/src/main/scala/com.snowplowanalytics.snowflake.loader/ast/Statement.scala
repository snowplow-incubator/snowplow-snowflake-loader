...
object Statement {
  ...
  implicit object CreateStageStatement extends Statement[CreateStage] {
    def getStatement(ddl: CreateStage): SqlStatement = {
      val auth = ddl.credentials.fold(""){
        case Auth.AwsKeys(accessKey, secretKey, token) =>
          if (token.isDefined) {
        ...
        case Auth.AwsRole(arn) => s" CREDENTIALS = ( AWS_ROLE = '$arn' )"
        case Auth.StorageIntegration(integrationName) => s" STORAGE_INTEGRATION = $integrationName"
      }
      SqlStatement(
  ...
  implicit object CopyInto extends Statement[CopyInto] {
    def getStatement(ast: CopyInto): SqlStatement = {
      ...
      val auth = ast.credentials.fold(""){
        ...
        case _ => "" // storage integration configured for the stage will authenticate implicitly
      }
  ...
  implicit object DescriveTableStatement extends Statement[DescribeTable] {
