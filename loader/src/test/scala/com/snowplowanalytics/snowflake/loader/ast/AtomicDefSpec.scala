...
class AtomicDefSpec extends Specification {
  ...
  "compare" should {
    ...
    "warn about unmatched columns" in {
      ...
      AtomicDef.compare(input) must beEqualTo(List("Existing column Column(unexpected,Variant,false,false) doesn't match expected definition Column(collector_tstamp,Timestamp,true,false) at position 4"))
    }
  }
}
