...
object SQLUtils {
  ...
  def createTable[F[_]: Sync](config: Config.Snowflake, xa: Transactor[F]): F[Unit] = {
    val t = fqTableName(config)
    Logger[F].info(s"Opening JDBC connection to ${config.url.getJdbcUrl}") *>
      xa.rawTrans.apply {
        Logger[ConnectionIO].info(s"Creating table $t if it does not already exist...") *>
          sqlCreateTable(t).update.run.void
      }
  }

  // fully qualified name
  def fqTableName(config: Config.Snowflake): String =
    s"${config.database}.${config.schema}.${config.table}"

  private def sqlCreateTable(tableName: String): Fragment = sql"""
