...
class KafkaConfigSpec extends Specification with CatsEffect {
  ...
  private def assert(resource: String, expectedResult: Either[ExitCode, Config[Unit, KafkaSourceConfig, KafkaSinkConfigM[Id]]]) = {
    val path = Paths.get(getClass.getResource(resource).toURI)
    ConfigParser.configFromFile[IO, Config[Unit, KafkaSourceConfig, KafkaSinkConfigM[Id]]](path).value.map { result =>
...
object KafkaConfigSpec {
  private val minimalConfig = Config[Unit, KafkaSourceConfig, KafkaSinkConfigM[Id]](
   ...
   */
  private val extendedConfig = Config[Unit, KafkaSourceConfig, KafkaSinkConfigM[Id]](
    input = KafkaSourceConfig(
      topicName             = "sp-dev-enriched",
      bootstrapServers      = "localhost:9092",
      debounceCommitOffsets = 10.seconds,
