...
class PubsubConfigSpec extends Specification with CatsEffect {
  ...
  private def assert(
    resource: String,
    expectedResult: Either[ExitCode, Config[PubsubFactoryConfig, PubsubSourceConfig, PubsubSinkConfigM[Id]]]
  ) = {
    val path = Paths.get(getClass.getResource(resource).toURI)
    ConfigParser.configFromFile[IO, Config[PubsubFactoryConfig, PubsubSourceConfig, PubsubSinkConfigM[Id]]](path).value.map { result =>
...
object PubsubConfigSpec {
  private val minimalConfig = Config[PubsubFactoryConfig, PubsubSourceConfig, PubsubSinkConfigM[Id]](
    input = PubsubSourceConfig(
      ...
      streamingPull           = true,
      retries                 = PubsubSourceConfig.Retries(PubsubSourceConfig.TransientErrorRetrying(100.millis, 10))
    ),
    output = Config.Output(
      ...
      bad = Config.SinkWithMaxSize(
        sink = PubsubSinkConfigM[Id](
          ...
          retries              = PubsubSinkConfig.Retries(PubsubSinkConfig.TransientErrorRetrying(100.millis, 10))
        ),
        maxRecordSize = 9000000
      )
    ),
    streams = PubsubFactoryConfig(
      emulatorHost = None
    ),
   ...
   */
  private val extendedConfig = Config[PubsubFactoryConfig, PubsubSourceConfig, PubsubSinkConfigM[Id]](
    input = PubsubSourceConfig(
      subscription            = PubsubSourceConfig.Subscription("myproject", "snowplow-enriched"),
      parallelPullFactor      = 0.5,
      durationPerAckExtension = 15.seconds,
