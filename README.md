...
docker run \
  ...
  snowplow/snowflake-loader-kafka:0.6.1 \
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  snowplow/snowflake-loader-pubsub:0.6.1 \
  --config /var/config.hocon
```
...
docker run \
  -v /path/to/config.hocon:/var/config.hocon \
  snowplow/snowflake-loader-kinesis:0.6.1 \
  --config /var/config.hocon
```
...
[release-image]: https://img.shields.io/badge/release-0.6.1-blue.svg?style=flat
